...
void modrm(const ModRMParams& params) {
  ...
  void complexmovregreg(char dest_reg, char src_reg, int32_t disp = 0, bool dest_is_mem = false) {
    ...
    if (disp == 0 && src_reg != 5) {  // No displacement, except when src_reg is [rbp]
      out.push_back(0x00 | (dest_is_mem ? src_reg << 3 : dest_reg << 3) | (dest_is_mem ? dest_reg : src_reg));
    } else if (disp >= -128 && disp <= 127) {
      out.push_back(0x40 | (dest_is_mem ? src_reg << 3 : dest_reg << 3) | (dest_is_mem ? dest_reg : src_reg));
      out.push_back(disp & 0xFF);
    } else {
      out.push_back(0x80 | (dest_is_mem ? src_reg << 3 : dest_reg << 3) | (dest_is_mem ? dest_reg : src_reg));
      for (int i = 0; i < 4; ++i) {
